...
class PayController extends HomebaseController {
	...
	public function notify_ali() {
		...
		if($verify_result) {//验证成功
			...
			if($_POST['trade_status'] == 'TRADE_FINISHED') {
				...
				$orderinfo=Db::name('charge_user')->where("orderno='{$out_trade_no}' and type='1'")->find();
	...
	*/
	...
	public function notify_ios(){
		...
		if($iforderinfo){
			echo '{"status":"fail","info":"'.lang('非法提交').'-001"}';
            exit;
		}
        
        $chargeinfo=Db::name("charge_rules")->where("product_id='{$info['product_id']}'")->find();
        if(!$chargeinfo){
            echo '{"status":"fail","info":"'.lang('非法提交').'-002"}';
            exit;
        }
        ...
        $this->logios("sql2:".Db::name('charge_user')->getLastSql());
		if($orderinfo){
            if($orderinfo['status']==0){
                /* 更新会员虚拟币 */
                $coin=$orderinfo['coin']+$orderinfo['coin_give'];
			...
			echo '{"status":"fail","info":"'.lang('订单信息不存在').'-003"}'; 		
			exit();
		}
		echo '{"status":"success","info":"'.lang('支付成功').'"}';
		exit;
	}		
	public function getReceiptData($receipt, $version_ios){ 
        ...
        $this->logios("ios_shelves:".$config['ios_shelves']);
        $ambient=0;
		...
		if($errno != 0) {   
			echo '{"status":"fail","info":"'.lang('服务器出错，请联系管理员').'"}';
			exit;
		}   
		$data = json_decode($response,1);   

		if (!is_array($data)) {   
			echo '{"status":"fail","info":"'.lang('验证失败，如有疑问请联系管理').'"}';
			exit;
		}   
