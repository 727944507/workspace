<?php /*a:2:{s:86:"D:\phpstudy_pro\WWW\yuejie.yunbaozb.com/themes/admin_simpleboot3/admin\cash\index.html";i:1603704624;s:83:"D:\phpstudy_pro\WWW\yuejie.yunbaozb.com/themes/admin_simpleboot3/public\header.html";i:1604631154;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
...
<body>
	<div class="wrap">
        ...
        <form class="well form-inline margin-top-20" name="form1"  method="post" action="">
			...
			提现类型：
            <select class="form-control" name="votes_type" style="width: 100px;">
                <option value=''>全部</option>
                <?php if(is_array($votestype) || $votestype instanceof \think\Collection || $votestype instanceof \think\Paginator): $i = 0; $__LIST__ = $votestype;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?>
                    <option value="<?php echo $key; ?>" <?php if(input('request.votes_type') != '' && input('request.votes_type') == $key): ?>selected<?php endif; ?>><?php echo $v; ?></option>
                <?php endforeach; endif; else: echo "" ;endif; ?>
            </select>
            提交时间：
            ...
            <input class="form-control" type="text" name="keyword" style="width: 200px;" value="<?php echo input('request.keyword'); ?>"
                   placeholder="订单号">
            <input type="submit" class="btn btn-primary" value="搜索" onclick="form1.action='<?php echo url('Cash/index'); ?>';form1.submit();"/>
			<input type="button" class="btn btn-success" value="导出" onclick="form1.action='<?php echo url('Cash/export'); ?>';form1.submit();"/>
            <a class="btn btn-danger" href="<?php echo url('Cash/index'); ?>">清空</a>
			<div class="admin_main" style="margin-top:10px">
				当前提现总金额：<?php echo $cash['total']; if($cash['type'] == '0'): ?>
					成功提现金额：<?php echo $cash['success']; ?>
					待处理金额：<?php echo $cash['fail']; ?>
				<?php endif; ?>
			</div>
        </form>
			...
			<table class="table table-hover table-bordered">
				...
				<tbody>
					...
					<tr>
						...
						<td><?php echo $votestype[$vo['votes_type']]; ?></td>						
						<td><?php echo $vo['votes']; ?></td>
						<td><?php echo $vo['money']; ?></td>
						<td><?php echo $type[$vo['type']]; ?><br><?php echo $vo['name']; ?><br><?php echo $vo['account']; ?><br><?php echo $vo['account_bank']; ?></td>
						...
						<td>
							<?php if($vo['status'] == '0'): ?>
							<a class="btn btn-xs btn-primary" href='<?php echo url("Cash/edit",array("id"=>$vo["id"])); ?>'><?php echo lang('EDIT'); ?></a>
							<?php endif; ?>
                           <!--  <?php if($vo['status'] == 0): ?>
							<a class="btn btn-xs btn-success js-ajax-dialog-btn" href="<?php echo url('Cash/setCash',array('id'=>$vo['id'],'status'=>'1')); ?>" data-msg="确定确认提现吗？">确认提现</a>
                            
                            <a class="btn btn-xs btn-danger js-ajax-dialog-btn" href="<?php echo url('Cash/setCash',array('id'=>$vo['id'],'status'=>'2')); ?>" data-msg="确定拒绝提现吗？">拒绝提现</a>
                            <?php endif; ?> -->
						</td>
					</tr>
					<?php endforeach; endif; else: echo "" ;endif; ?>
