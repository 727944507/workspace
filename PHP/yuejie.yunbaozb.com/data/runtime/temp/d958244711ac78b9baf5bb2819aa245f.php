<?php /*a:1:{s:70:"D:\phpstudy_pro\WWW\yuejie.yunbaozb.com/plugins/qiniu/view/upload.html";i:1603704622;}*/ ?>
<!doctype html>
<html>
<head>
    <meta charset="utf-8">
    ...
    <?php endif; ?>
    <link rel="stylesheet" type="text/css" href="/public/static/js/webuploader/webuploader.css">
    <link rel="stylesheet" type="text/css" href="/public/static/js/webuploader/image-upload/style.css">
    <style>
...
<div class="wrap " style="padding:5px;">
    <ul class="nav nav-tabs">
        <?php 
         ...
         ?>
        <li class=""><a href="<?php echo url('user/Asset/webuploader',$url_params); ?>">上传文件</a></li>
        <?php 
            $url_params['tab']='url';
         ?>
        <li class=""><a href="<?php echo url('user/Asset/webuploader',$url_params); ?>">网络文件</a></li>
...
<script src="/public/static/js/webuploader/webuploader.min.js"></script>
    ...
    (function ($) {
        ...
        $(function () {
            ...
            WebUploader.Uploader.register({
                'add-file': function (files) {

                },
                'before-send': function (block) {
                    var server = '//<?php echo $upload_host; ?>/mkblk/' + block.blob.size;
            ...
            uploader = WebUploader.create({
                ...
                formData: {},
                headers: {
                    Authorization: 'UpToken <?php echo $qiniu_up_token; ?>'
                },
                accept: {
                ...
                chunked: true,//开启分片
                auto: true,
                chunkSize: 4 * 1024 * 1024,// 单位B
                compress: false,
                server: "//<?php echo $upload_host; ?>/mkblk/4194304",
                ...
                switch (type) {
                    case 'uploadSuccess':
                        if (msg._raw != '') {
                            ...
                            $.ajax({
                                url: '//<?php echo $upload_host; ?>/mkfile/' + file.size,
                                type: 'POST',
                                dataType: 'json',
                                contentType: 'text/plain',
                                data: fileCtxes[file.id].join(','),
                                headers: {Authorization: 'UpToken <?php echo $qiniu_up_token; ?>'},
                                success: function (data) {
                                    var $file = jQuery("#" + file.id);
                                    $file.data("name", file.name);
                                    $.ajax({
                                        url: "<?php echo cmf_plugin_url('Qiniu://Asset/getUrl'); ?>",
                                        type: 'POST',
                                        dataType: 'json',
                                        data: {
                                            filename: file.name,
                                        ...
                                        success: function (data2) {
                                            $file.data("url", data2.data.url);
                                            $file.data("preview_url", data2.data.preview_url);
                                            $file.data("filepath", data2.data.filepath);
                                        },
                                        error: function () {
                                    ...
                                    $.ajax({
                                        url: "<?php echo cmf_plugin_url('Qiniu://Asset/saveFile'); ?>",
                                        type: 'POST',
                                        dataType: 'json',
                                        data: {
                                            filename: file.name,
            ...
            uploader.on("uploadAccept", function (object, ret) {
                ...
                console.log(ret);
                if (ret.ctx) {
                    if (!fileCtxes[object.file.id]) {
                        fileCtxes[object.file.id] = [];
                    }
                    fileCtxes[object.file.id].push(ret.ctx);
                    return true;
                } else {
                    return false;
                }
